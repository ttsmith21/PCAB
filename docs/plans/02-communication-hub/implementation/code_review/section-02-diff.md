diff --git a/components/ui/SignupForm.tsx b/components/ui/SignupForm.tsx
new file mode 100644
index 0000000..45cc0d5
--- /dev/null
+++ b/components/ui/SignupForm.tsx
@@ -0,0 +1,387 @@
+"use client";
+
...
+    }
+    if (phone.trim() && !commPref) {
+      errs.commPref = "Please select a communication preference";
+    }
+    return errs;
+  }
+
...
+            {levels.map((level) => {
+              const optionId =
+                MAILCHIMP_GROUPS.level.options[level.value] || level.value;
+              return (
+                <label
+                  key={level.value}
+                  className="flex items-center gap-2 cursor-pointer"
